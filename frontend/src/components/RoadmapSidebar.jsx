...
export default function RoadmapSidebar({ 
    ...
    // Memoize flattenNodes to prevent unnecessary recalculations
    const flattenNodes = useCallback((node, parentTitle = '') => {
        let flatNodes = [];
        ...
        if (node.children) {
            node.children.forEach(child => {
                flatNodes = [...flatNodes, ...flattenNodes(child, node.title)];
            });
        }

        return flatNodes;
    }, [getNodeStatus, roadmapId]);

    // Update local nodes only when necessary
    useEffect(() => {
        const flattened = flattenNodes(nodes);
        setLocalNodes(prev => {
            // Only update if the nodes have actually changed
            if (JSON.stringify(prev) !== JSON.stringify(flattened)) {
                return flattened;
            }
            return prev;
        });
    ...
    const handleNodeToggle = async (nodeId) => {
        try {
            ...
            await updateNodeProgress(roadmapId, nodeId, !isCurrentlyCompleted, localNodes.length);
